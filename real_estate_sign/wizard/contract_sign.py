...
class RentalSign(models.TransientModel):
    ...
    _description = "Sign Documents from a contract"

    @api.model
    def default_get(self, fields):
        res = super(RentalSign, self).default_get(fields)
        if 'template_id' in fields:
            company = self.env['realestate.contract.model'].browse(res.get('order_id')).company_id or self.env.company
    ...
    contract_id = fields.Many2one(
        "realestate.contract.model", "Contract", required=True, ondelete="cascade",
    ...
    def next_step(self):
        pending_sign_request = self.contract_id.sign_request_ids.filtered(
        ...
        if pending_sign_request:
            ...
            action["context"] = {
                ...
                "default_sale_order_id": self.contract_id.id,
            }
            return action
